...
public class HealthProfileReader {
	...
	static
    {
		...
		Person tony = new Person(new Long(30),"Tony","Toms","1999-11-11",hp);
        database.put(tony.getPersonId(), tony);
    ...
    public static void updateHealthProﬁle(Long	personId, Double height, Double	weight) {
        Person p = database.get(personId);
        if (p==null)
            p = new Person(personId,"Finch","morris","19-01-1020");
    ...
    public static void getBMI(Long personId) {
        Person p = database.get(personId);
        double bmi = p.gethProfile().getWeight()/(p.gethProfile().getHeight() * p.gethProfile().getHeight());
        System.out.println(p.getFirstname()+ " " + p.getLastname() + " has a BMI equal to" + bmi);
    }
}